...
1. If the vault operating manual (`_CLAUDE.md`) is not already in your context, read it from the vault root. Resolve the destination folder per `references/folder-map.md` (a distillation is a concept/reference note: wiki-style `wiki/concepts/`, Obsidian-style `Knowledge/`).
